...
if __name__ == "__main__":
    ...
    new_csv_folder_path = os.path.abspath(os.path.join(csv_folder_path, os.pardir, "cosine_similarity_5sentnces", city))
        ...
        if file_name.endswith(".csv"):
            ...
            try:
                ...
                if len(texts) == 0:
                    ...
                    grouped_texts=grouping_sentences(texts)
                    cosine_list=[]
                    for article in grouped_texts:
                        embeddings=model.encode(texts)
                        cosine_similarity_scores = cosine_similarity([input_vector], embeddings)
                        cosine_list.append(cosine_similarity_scores[0])
                    sentence_df=pd.DataFrame.from_dict({"text": grouped_texts, "similarity": cosine_list})
                    sentence_df['file']=file_name
                    new_file_path = os.path.join(new_csv_folder_path, file_name.replace(".csv", "_5sentences_cosine_similarity.csv"))
                    sentence_df.to_csv(new_file_path, index=False)
                    break
            except:
